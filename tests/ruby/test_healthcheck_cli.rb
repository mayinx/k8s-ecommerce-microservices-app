...
#
# DESCRIPTION:
#   Integration tests for the Healthcheck CLI executable.
...
class HealthcheckCliTest < Minitest::Test
  ...
  # TEST: Verify that the script aborts gracefully when missing required arguments.
  #
  # CONCEPT: SUB-PROCESS TESTING (Open3)
  # Instead of instantiating a Ruby class, we use `Open3.capture3` to spawn a 
  # completely separate operating system process. This allows us to capture 
  # exactly what a user would see in their terminal (stdout/stderr) and 
  # how the script reports its success/failure to the OS (exit status).
