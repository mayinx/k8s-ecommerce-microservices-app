...
Before introducing any test framework or workflow changes, we need to identify 

- (1) which **repository-owned code** is actually **suitable for testing** in this project and 
- (2) which relevant **validation gap** is still **not covered by those owned helpers**

#### Suitable repo-owned test candidates

At this point, the strongest owned executable test surfaces are:

- **Ruby Healthcheck**
    - `healthcheck/healthcheck.rb`
- **Traffic Generator as Observability Helper**
...
- The Traffic Generator was introduced in Phase 06 as a custom observability helper 
- Both files are executable helpers with real control flow and logic that can be tested

#### Validation Gap

At the same time, those owned helpers do not cover all relevant quality layers by themselves:

- The Ruby helper primarily validates **service health/reachability**
  - it communicates with **health API endpoints** such as `catalogue/health`, `user/health`, and `carts/health`
  - it checks their reachability / health status
  - and aggregates the results into a health status report printed in the terminal    
- The Bash helper primarily covers **traffic generation on the live edges for observability** (as implemented and docuemnetd in Phase 06 - Oberservability)
- **What is still missing **is a **repo-owned schema-validation layer** for the **structure of API responses** consumed by downstream services such as the `front-end` - for example, to **validate the expected response structure of endpoints** like `/catalogue`.

#### Python QA utility module: API contract guard for response-schema validation

To **close this validation gap**, Phase 07 will therefore also implement a small **Python QA utility module** in the form of a **Python API contract guard** as an additional repo-owned test surface. 

This gives the project a **dedicated response-schema validation layer** as valuable QA addition - and a **Python unit-test path**.

### Action

**Basic Test Scaffold**

After the test surface is decided, we need to create a corresponding **basic testing scaffold** that **separates the testing concerns** cleanly:   
...
We also include an `e2e` scaffold from the beginning. This reserves space for a later **browser-level validation layer**, because API and helper checks alone do not prove that the deployed storefront actually renders correctly in a real browser.

**`.gitignore`**

Once that structure exists, `.gitignore` must be extended to exclude the usual test dependencies and generated test artifacts that should not be committed to version control:
...
## Step 2 — Assess the repo-owned helper surfaces, proving their current behavior, and identifying the testability flaws
...
#### Functional Check I: Assessing current state locally 
...
#### Functional Check: Assessing the current state (from repo root)
...
Step 2 shows that both selected repo-owned helpers are **functionally valid in their intended operational behavior**, but are **not yet structured cleanly for framework-driven automated tests**:
...
### Creating the first Ruby characterization test

Before changing the helper structure, a small CLI-level test helper needs to be created to verify the **currently intended behavior** (the "original character") of the unrefactored script:  
...
To run this characterization test:
...
This ensures, that the Traffic Generator's behavior is preserved through the refactoring phase:   

### Refactoring the helper into a testable class with an execution guard

Once the characterization test passes, we replace the current `healthcheck/healthcheck.rb` with the following minimally refactored version.
...
#### Removing the dynamic Ruby gem installation step (`awesome_print`) from `run-healthcheck-target-env.sh`
...
#### Removing the `awesome_print` gem from the Healtcheck `Dockerfile` 
...
### Refactoring the Healtcheck Target Proof-Helper (`run-healthcheck-target-env.sh`)
...
### Adding Ruby characterization and unit tests
...
#### Running the Ruby characterization and unit-test suites
...
#### Confirming the current CLI behavior with two black-box checks
